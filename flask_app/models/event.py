...
class Event:
    ...
    def create_event(cls, form_data):
        query = "INSERT INTO events (event_type, address, city, state, spots_open, event_date, event_time, user_id) VALUES (%(event_type)s, %(address)s , %(city)s, , %(state)s %(spots_open)s, %(event_date)s, %(event_time)s,  %(user_id)s);"
    ...
    def get_all_events_with_user_by_event_location(cls, form_data):
        query = "SELECT * FROM events LEFT JOIN users on events.user_id = users.id WHERE state = %(state)s;"
        events = connectToMySQL('group_project_schema').query_db(query, form_data)
        results = []
        for event in events:
            data = {
    ...
    def update_event(cls, form_data):
        query = "UPDATE events SET event_type=%(event_type)s, address=%(address)s, city=%(city)s, state=%(state)s, spots_open=%(spots_open)s, event_date=%(event_date)s, event_time=%(event_time)s WHERE id=%(id)s;"
        return connectToMySQL('group_project_schema').query_db(query, form_data)
